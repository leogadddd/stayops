...
export default async function EditReservationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl"><PageHeading title="Edit reservation" description="Update booking details. Payment history stays as an immutable ledger." backHref={`/reservations/${id}`} backLabel="Back to reservation" /><Card><CardBody><EditReservationForm reservationId={id} unitId={detail.unit.id} capacity={detail.unit.capacity} checkIn={detail.reservation.checkInDate} checkOut={detail.reservation.checkOutDate} guestCount={detail.reservation.guestCount} occupants={detail.occupants.map((occupant) => occupant.name)} guestId={detail.guest.id} guests={guests.map((guest) => ({ id: guest.id, name: guest.name }))} units={units.filter((unit) => unit.status === "active" || unit.id === detail.unit.id).map((unit) => ({ id: unit.id, label: label(unit), capacity: unit.capacity }))} charges={detail.charges} /></CardBody></Card></div>;
}
